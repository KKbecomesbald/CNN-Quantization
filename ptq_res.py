...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load('ckpt/CIFAR10_resnet18.pth'))
    
    #eport onnx model of resnet
    x = torch.randn(BATCH_SIZE, 3, 32, 32, requires_grad=True)
    torch.onnx.export(model,
                      x,
                      "ckpt/resnet18_self_defined.onnx",
                      export_params=True,
                      opset_version=12,
    ...
    model_file = 'ckpt/resnet18_self_defined.onnx'
    onnx_model = onnx.load(model_file)
    onnx.save(onnx.shape_inference.infer_shapes(onnx_model), model_file)

    save_file = 'ckpt/CIFAR10_resnet18_ptq.pth'
